...
## Author:              Renze Jongman, Fabio Beltran
## Date:                28 June 2018
...
class generateEvents():
    ...
    def initMISP(self):
        self.misp   = PyMISP(self.url, self.key, False, 'json')
    ...
    def addEvents(self):
        for i in range(len(self.paste)):
            if len(self.paste[i].iocs) != 0:
                ...
                #event   = self.misp.MISPEvent(distribution=2, analysis=2, info=self.paste[i].title)
                event   = MISPEvent()
                event.info=self.paste[i].title
                event.distribution = 2
                event.analysis=2
                #self.misp.add_internal_link(event, self.paste[i].URI, category="External analysis")
                event.add_attribute('link',self.paste[i].URI,category="External analysis")
                #self.misp.add_tag(event, "Type:OSINT")
                event.add_tag(tagosint)
                ...
                for j in range(len(self.paste[i].iocs)):
                    if self.paste[i].iocs[j].kind   == "IP":
                        ...
                        event.add_attribute('link',self.paste[i].iocs[j].value,category="Network activity")
                    elif self.paste[i].iocs[j].kind == "uri":
                        #self.misp.add_url(event, self.paste[i].iocs[j].value)
                        event.add_attribute('url',self.paste[i].iocs[j].value,category="Network activity")                     
                    elif self.paste[i].iocs[j].kind == "md5":
                        #self.misp.add_hashes(event, md5=self.paste[i].iocs[j].value)
                        event.add_attribute('md5',self.paste[i].iocs[j].value,category="Payload delivery")
                    elif self.paste[i].iocs[j].kind == "sha1":
                        #self.misp.add_hashes(event, sha1=self.paste[i].iocs[j].value)
                        event.add_attribute('sha1',self.paste[i].iocs[j].value,category="Payload delivery")
                    
                    elif self.paste[i].iocs[j].kind == "sha256":
                        #self.misp.add_hashes(event, sha256=self.paste[i].iocs[j].value)
                        event.add_attribute('sha256',self.paste[i].iocs[j].value,category="Payload delivery")
                    
                    elif self.paste[i].iocs[j].kind == "CVE":
                        #self.misp.add_object(event, 63, self.paste[i].iocs[j].value)
                        event.add_attribute('vulnerability',self.paste[i].iocs[j].value,category="External analysis")
                        
                    if self.paste[i].iocs[j].kind   == "email":
                        #self.misp.add_email_src(event, self.paste[i].iocs[j].value)
                        event.add_attribute('email-src',self.paste[i].iocs[j].value,category="Payload delivery")
                    
                    if self.paste[i].iocs[j].kind   == "filename":
                        #self.misp.add_filename(event, self.paste[i].iocs[j].value)
                        event.add_attribute('filename',self.paste[i].iocs[j].value,category="Payload delivery")
                
                
                if PUBLISH_EVENTS:
                    self.misp.publish(event, alert=EMAIL_ALERTS)
                
                event = x.misp.add_event(event, pythonify=True)
